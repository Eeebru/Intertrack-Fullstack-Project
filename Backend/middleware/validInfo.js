module.exports = (req, res, next) => {
  ...
  if (req.path === '/signup') {
    if (![email, name, password, confirmPassword, gender, dob, address, state].every(Boolean)) {
      return res.status(400).json({status: 400, result: 'One or More Credential is Missing'});
