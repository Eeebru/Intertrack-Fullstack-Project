...
router.post('/orderproduct', auth, async (req, res) => {
  ...
  try {
    ...
    const uniqueOrder = await db.query(`SELECT * FROM public."order" WHERE user_id = $1`, [req.id]);
    const userOrders = uniqueOrder.rows;
    userOrders.map(order => {
      if (order.product_id === product_id && order.expiry_date > new Date() ) {
        res.status(401).json({message: 'You already have this plan'})
      }
    })
